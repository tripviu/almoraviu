...
const KEY = "Almoraviu_wishlist_v1";
...
export function toggleWishlist(id: string){
  ...
  window.dispatchEvent(new CustomEvent("Almoraviu:wishlist:changed"));
}
export function onWishlistChange(handler: ()=>void){
  if (typeof window === "undefined") return () => {};
  const fn = () => handler();
  window.addEventListener("Almoraviu:wishlist:changed", fn);
  return () => window.removeEventListener("Almoraviu:wishlist:changed", fn);
}
